...
## STATUS: READY
**Created**: 2026-02-17 | **Context**: AI Provider Integration (Inference + Embeddings)
...
## ACCEPTANCE CRITERIA
- [ ] Se puede seleccionar `vllm` como `LLM_PROVIDER` y como `EMBEDDING_PROVIDER` desde settings.
- [ ] `get_llm_client()` crea cliente OpenAI-compatible apuntando a `VLLM_BASE_URL` para chat/completions.
- [ ] `create_embeddings_batch()` funciona con vLLM vía `/v1/embeddings` usando modelo configurable.
- [ ] Validación de provider/modelo incluye `vllm` en backend y frontend sin romper providers actuales.
- [ ] Errores de conexión/timeouts del backend vLLM se reportan con excepciones tipadas y contexto útil.
- [ ] `.env.example` y docs reflejan configuración vLLM (URL, key opcional, modelos chat/embedding).
- [ ] Flujo backward-compatible: providers existentes siguen operando sin cambios.
